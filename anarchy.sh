...
echo "anarchaos: ICMP, SSH, HTTP, HTTPS, ELK stack[Dev cloud]"
...
#ElasticSearch
iptables -t mangle -A INPUT -p tcp --dport 9200 -j ACCEPT
iptables -t mangle -A OUTPUT -p tcp --sport 9200 -m state --state=ESTAB,REL -j ACCEPT
iptables -t mangle -A INPUT -p tcp --dport 9300 -j ACCEPT
iptables -t mangle -A OUTPUT -p tcp --sport 9300 -m state --state=ESTAB,REL -j ACCEPT

#LogStash
iptables -t mangle -A INPUT -p tcp --dport 9600 -j ACCEPT
iptables -t mangle -A OUTPUT -p tcp --sport 9600 -m state --state=ESTAB,REL -j ACCEPT

#Kibana
iptables -t mangle -A INPUT -p tcp --dport 5601 -j ACCEPT
iptables -t mangle -A OUTPUT -p tcp --sport 5601 -m state --state=ESTAB,REL -j ACCEPT
